...
4. **To run a node manually**: click a node in the graph, then **▶ Run selected** in the toolbar for a background run. Keep the node selected to watch its **Run Output** panel, right-click the node and choose **Open chat window** for a full transcript tab, or choose **Force stop** to cancel a running background node. Type `@orchestrator /run sec` in the native chat panel (Ctrl+Alt+I) for an interactive chat run.

## Phase-1 verification (the load-bearing test)

Click the SecAgent node, then click ▶ Run selected. The node should run in the extension host, record activity in the graph/ledger, and show its captured transcript in the selected node's **Run Output** panel without opening the native chat panel. Use **Open chat** or right-click → **Open chat window** for a full editor-tab transcript. Right-click → **Force stop** should cancel a running background node and mark it stopped. Type `@orchestrator /run sec` in chat when you specifically want an interactive chat-thread run.
